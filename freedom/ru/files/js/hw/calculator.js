...
function drawCalculator() {
	...
	$("#points>.points2>.count").text(MAX_POINTS);
	
	$("#points>.points2>.count2").text(MAX_POINTS2);
	$("#points>.points2>.count4").text(MAX_POINTS3);
	
	$("#points>.count3").text(MAX_UROVEN);
...
function masteryTooltip(tree, index, rank) {
	...
	var text = {
		tree: tree,
		header: mastery.name,
		rank: "Уровень: " + rank + "/" + mastery.ranks,
...
function masteryTooltipReq(tree, index) {
	...
	if (pointReq > treePoints(tree))
		missing.push("Требуется очков в ветке талантов " + treeNames[tree][0].toUpperCase() + treeNames[tree].slice(1) + ": " + pointReq);
...
function masteryTooltipReq2(tree, index) {
	...
	if (!masteryParentReq(tree, index)) {
		var parent = data[tree][index].parent;
		missing.push("Требуется очков в таланте " + data[tree][parent].name + " не менее: " + data[tree][parent].ranksnw);
	}
...
function updateLabels() {
	for (var tree=0; tree<3; tree++) {
		...
		$("#points>.points2>.count").text(MAX_POINTS + totalPoints);
		
		$("#points>.points2>.count2").text(MAX_POINTS2 + totalPoints);
		$("#points>.points2>.count4").text(MAX_POINTS3 + totalPoints);
		
		$("#points>.count3").text(MAX_UROVEN + totalPoints);
	}
}
	...
	for (var tree = 0; tree < 3; tree++) {
		$("#panel>#tree-summaries").append(
			$("<div>")
				...
				.text(0)
				.css({
					left: TREE_OFFSET * tree +254,
					top: 66,
					cursor: "pointer",
				})
				.mouseover(function(){
					customTooltip($("#tooltip").show(), "Двойной клик для очистки ветки");
				})
				.mouseout(function(){
					$("#tooltip").hide();
				})
