...
function masteryTooltipReq(tree, index) {
    ...
    if (pointReq > treePoints(tree))
        missing.push("Requiere " + pointReq + " puntos en el árbol de " + treeNames[tree][0].toUpperCase() + treeNames[tree].slice(1));
...
function masteryTooltipReq2(tree, index) {
    ...
    if (!masteryParentReq(tree, index)) {
        var parent = data[tree][index].parent;
        missing.push("Requiere que el " + data[tree][parent].name + " de talento tenga al menos " + data[tree][parent].ranksnw + " de puntos");
    }
    ...
    for (var tree = 0; tree < 3; tree++) {
        $("#panel>#tree-summaries").append(
            $("<div>")
                ...
                .text(0)
                .css({
                    left: TREE_OFFSET * tree +287,
					top: 66,
                    cursor: "pointer",
                })
                .mouseover(function(){
                    customTooltip($("#tooltip").show(), "Haga doble clic para restablecer árbol");
                })
                .mouseout(function(){
                    $("#tooltip").hide();
                })
