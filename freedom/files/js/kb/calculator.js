...
function masteryTooltipReq(tree, index) {
    ...
    if (pointReq > treePoints(tree))
        missing.push("Requires " + pointReq + " points in the tree " + treeNames[tree][0].toUpperCase() + treeNames[tree].slice(1));
...
function masteryTooltipReq2(tree, index) {
    ...
    if (!masteryParentReq(tree, index)) {
        var parent = data[tree][index].parent;
        missing.push("Requires talent " + data[tree][parent].name + " to have at least " + data[tree][parent].ranksnw + " point(s)");
    }
